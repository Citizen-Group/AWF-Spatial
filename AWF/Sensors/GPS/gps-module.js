 ...
 *
 * Expected input is a NMEA String over TCP.
 *
 ...
 * - node-nmea: GPS nmea code parser
 *
 * Update the config to connect to your mobile.
 */
...
var gps = function() {
    ...
    /**
     * Upon initial connection. Create a 2 second heartbeat
	 * to display in the node terminal.
     */
     ...
     * connection.
     */
	client.on('data', function(imcomingData) {

	    // Incomming data is a full NMEA string. Break up the data by line.
		var data = imcomingData.toString('utf8');
		data = data.split('\r\n');

		// For each line in the NMEA iterate.
		for (let i in data) {
			...
			if (data[i].includes('$GPRMC')) {
				...
				if (internal_values[3] == "" || internal_values[3] == " ") {
